...
function RegisterForm({ setFocus, focus, openLogin, setLogin }: Form) {
  ...
  return (
    <CreateAccountContainer openLogin={openLogin.register}>
      <LoginMessage
        style={{
          display: openLogin.register ? "block" : "none",
          color: message.includes("succes") ? "green" : "#e32636",
        }}
      >
        {message}
      </LoginMessage>
      ...
      <InputForm onSubmit={handleSubmit(onSubmitLogin)}>
        <InputContainer openLogin={openLogin.register}>
          <InputWrapper>
            <InputLabel
              className={
                focus.registerEmail || inputs.registerEmail ? "focus" : ""
              }
            >
              Email
            </InputLabel>
            <Input
              ...
              onFocus={() => setFocus({ ...focus, registerEmail: true })}
              onBlur={() => setFocus({ ...focus, registerEmail: false })}
            />
            <p style={{ color: "red", marginTop: "0.8rem" }}>
              {errors?.registerEmail?.message as string}
            </p>
          </InputWrapper>
          <div className="register">
            <InputWrapper>
              <InputLabel
                className={focus.firstName || inputs.firstName ? "focus" : ""}
              >
                Prenume
              </InputLabel>
              <Input
            ...
            <InputWrapper>
              <InputLabel
                className={focus.lastName || inputs.lastName ? "focus" : ""}
              >
                Nume
              </InputLabel>
              <Input
          ...
          <InputWrapper>
            <InputLabel
              className={
                focus.registerPassword || inputs.registerPassword ? "focus" : ""
              }
            >
              Parola
            </InputLabel>
            <Input
              ...
              onFocus={() => setFocus({ ...focus, registerPassword: true })}
              onBlur={() => setFocus({ ...focus, registerPassword: false })}
            />
            <p style={{ color: "red", marginTop: "0.8rem" }}>
